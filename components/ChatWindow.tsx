...
const ChatWindow: React.FC<ChatWindowProps> = ({ 
  ...
  }, [messages, isTyping, liveInputTranscription, liveOutputTranscription, isTranscribing]);
  ...
  const processUserMessage = async (text: string) => {
    ...
    onSendMessage({ sender: 'user', content: text, type: 'text', timestamp: new Date() });
    setIsTyping(true);
    
    try {
      const isImageRequest = /(gere|crie|desenhe|faca|gera)\s+(uma|um)?\s*(imagem|logo|mockup|layout|arte|ilustracao)/i.test(text);
      if (isImageRequest) {
      ...
      const stream = await getAgentResponseStream(
        activeAgent, 
        text, 
        messages.map(m => ({ role: m.sender === 'user' ? 'user' : 'model', parts: [{ text: m.content }] })) as any, 
        context,
        voicePreference
      );
  ...
  return (
    <div className="flex flex-col h-full bg-[#020617] relative w-full overflow-hidden font-sans">
      ...
      {!isLive && (
        <footer className="px-4 py-4 sm:px-6 flex items-end gap-3 bg-[#0f172a] border-t border-slate-800 shrink-0 z-20">
          <textarea
            ref={textareaRef}
            value={inputText}
            onChange={(e) => setInputText(e.target.value)}
            onKeyDown={(e) => { if (e.key === 'Enter' && !e.shiftKey) { e.preventDefault(); processUserMessage(inputText); setInputText(''); } }}
            placeholder={isRecording ? "Gravando..." : "Conversar com amor..."}
